...
class DevDocker {
    ...
    async removeImages(
        ...
        const containerInfos = (await this.getContainerInfos()).filter(info => {
            return nameMatches.find(match =>
                DevDocker.containersImageMatched(info, match),
            )
        })
        for (let i = 0; i < containerInfos.length; i += 1) {
            ...
            progress(
                terminal,
                `Removing container [${DevDocker.containerTitle(info)}]`,
            )
        ...
        const imageInfos = (await this.getImageInfos()).filter(info => {
            return nameMatches.find(match =>
                DevDocker.imageHasMatchedName(info, match),
            )
        })
    ...
    async findImageInfo(name: string): Promise<ImageInfo | null> {
        return (
            (await this.getImageInfos()).find(x =>
                DevDocker.imageHasMatchedName(x, name),
            ) || null
        )
    }
    ...
    async findContainerInfo(name: string): Promise<ContainerInfo | null> {
        return (
            (await this.getContainerInfos()).find(x =>
                DevDocker.hasName(x, name),
            ) || null
        )
    }
    ...
    async startupContainer(
        terminal: Terminal,
        def: ContainerDef,
        upTo: ContainerStatus,
    ) {
        let info: ContainerInfo | null = await this.findContainerInfo(
            def.containerName,
        )
    ...
    static hasName(container: ContainerInfo, name: string): boolean {
        ...
        return !!(container.Names || []).find(
            n => n.toLowerCase() === nameToFind,
        )
    }
    ...
    static imageHasMatchedName(info: ImageInfo, match: string): boolean {
        return !!DevDocker.imageNames(info).find(name =>
            this.imageNameMatched(name, match),
        )
    }
