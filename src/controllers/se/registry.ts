...
export function seSourceDockerContainer(container: string, image: string): SESource {
    return {
        type: SESourceType.DOCKER_CONTAINER,
        container,
        image,
...
export class SERegistry {
        ...
        try {
            if (fs.existsSync(registryPath())) {
                ...
                this.items.forEach((instance) => {
                    const deprecated = instance as unknown as { version?: string };
    ...
    async getItemInfo(item: SERegistryItem): Promise<SERegistryItemInfo> {
        ...
        const info: Dockerode.ContainerInfo | null = await this.getDocker().findContainerInfo(def.containerName);
        return {
    ...
    async configure(terminal: Terminal, args: {
        version: string,
        image: string,
        container: string,
        port: string,
        ...
        if (args.version !== "") {
            ...
            const docker = this.getDocker();
            const image = (await docker.findContainerInfo(args.container))?.Image;
            if (image === undefined) {
