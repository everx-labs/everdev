...
import {changeExt, uniqueFilePath, writeStringToFile} from "../../core/utils";
import fs from "fs";
...
export const clangCreateCommand: Command = {
    ...
    async run(terminal: Terminal, args: { name: string; folder: string }) {
        ...
        writeStringToFile(hFilePath, BasicContractHeaders);
        writeStringToFile(cFilePath, BasicContractCode.split("{name}").join(hFilePath));
