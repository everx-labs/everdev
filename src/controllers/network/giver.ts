...
export class NetworkGiver implements AccountGiver {
    ...
    static async create(
        ...
        const signer =
            signerName !== ""
                ? await new SignerRegistry().resolveSigner(signerName, {
        ...
        const contract = await knownContractFromAddress(
            client,
            "Giver",
            address,
        )
