...
async function writeEnterMessage(
    docker: DevDocker,
    terminal: Terminal,
    containerName: string,
) {
    const containerInfo: ContainerInfo | null = await docker.findContainerInfo(
        containerName,
    )
    if (null !== containerInfo) {
        terminal.log(
            `Open in browser: http://localhost:${containerInfo.Ports[0].PublicPort}/`,
        )
    }
}
...
export const startCommand: Command = {
    ...
    async run(terminal: Terminal, args: { version: string }): Promise<void> {
        await controlInstances(async (docker, def) => {
            const containerInfo: ContainerInfo | null =
                await docker.findContainerInfo(def.containerName)
