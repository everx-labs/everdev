...
import { downloadDemo, loadInfo } from "./installer"
import { formatTable, writeTextFile } from "../../core/utils"
...
export const jsCreateCommand: Command = {
    ...
    async run(terminal: Terminal, args: { name: string; folder: string }) {
        ...
        writeTextFile(path.resolve(appFolderPath, "index.js"), BaseApp.index)
        writeTextFile(
            path.resolve(appFolderPath, "package.json"),
            BaseApp.package,
        )
...
export const jsDemoCommand: Command = {
    ...
    async run(terminal: Terminal, args: { name: string; folder: string }) {
        if (args.name === "") {
            const table = [
                ["Demo", "Description"],
                ...(await loadInfo()).applications.map(x => [
                    x.name,
                    x.description,
                ]),
            ]
...
export const JsApps: ToolController = {
    ...
    commands: [jsCreateCommand, jsDemoCommand, jsWrapCommand],
}
