...
export function formatTokens(nanoTokens: string | number | bigint): string {
    ...
    const tokens =
        Number(bigNano / token) + Number(bigNano % token) / Number(token)
    const tokensString =
        tokens < 1 ? tokens.toString() : `≈ ${Math.round(tokens)}`
...
function downloadAndGunzip(
    ...
    return new Promise((resolve, reject) => {
        const request = https.get(url, response => {
            ...
            let file: fs.WriteStream | null = fs.createWriteStream(dest, {
                flags: "w",
            })
            let opened = false
...
export function parseNanoTokens(
...
export function breakWords(s: string, maxLen = 80): string {
    let result = ""
